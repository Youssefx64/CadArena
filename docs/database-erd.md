...
## نظرة عامة
قاعدة البيانات الفعلية هي SQLite واحدة (`workspace.db`) وتضم المستخدمين والملفات الشخصية ومفاتيح المزودات والمشاريع والرسائل. العلاقات المضمونة على مستوى SQLite هي FK بين profile / provider keys / messages، بينما `projects.user_id` يبقى رابطًا تطبيقيًا فقط.
