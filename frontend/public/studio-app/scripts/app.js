...
async function uploadDxfAndView(file) {
  ...
  try {
    ...
    if (!fileToken) {
      throw new Error("No file token returned from upload");
    }
    ...
    if (dxfRenderEmpty) {
      dxfRenderEmpty.style.display = "grid";
      dxfRenderEmpty.innerHTML = '<div class="dxf-drop-card"><div class="dxf-drop-icon">ERR</div><p class="dxf-drop-title">Upload failed</p><p class="dxf-drop-hint">' + error.message + '</p></div>';
    }
  }
}
...
if (dxfRenderCanvas) {
  dxfRenderCanvas.addEventListener("dragover", (event) => {
    event.preventDefault();
    dxfRenderCanvas.classList.add("dxf-drag-active");
  });

  dxfRenderCanvas.addEventListener("dragleave", (event) => {
    if (!dxfRenderCanvas.contains(event.relatedTarget)) {
      dxfRenderCanvas.classList.remove("dxf-drag-active");
    }
  });

  dxfRenderCanvas.addEventListener("drop", async (event) => {
    event.preventDefault();
    dxfRenderCanvas.classList.remove("dxf-drag-active");
